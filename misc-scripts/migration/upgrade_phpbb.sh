...
new_phpbb_major=${new_phpbb_version%'.'*}
new_phpbb_minor=${new_phpbb_version##*'.'}
...
if [ "$test" ]; then
    ...
    curl https://download.phpbb.com/pub/release/$new_phpbb_major/$new_phpbb_version/$phpbb_archive -o "$HOME/$phpbb_archive"

    sudo mv phpbb phpbb.old
    sudo tar -xjf "$HOME/$phpbb_archive"
